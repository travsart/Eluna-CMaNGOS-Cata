...
struct boss_anubrekhanAI : public ScriptedAI
{
    ...
    void KilledUnit(Unit* pVictim) override
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_PLAYER)
            pVictim->CastSpell(pVictim, SPELL_SELF_SPAWN_5, TRIGGERED_OLD_TRIGGERED, NULL, NULL, m_creature->GetObjectGuid());
    ...
    void SummonedCreatureDespawn(Creature* pSummoned) override
    {
        ...
        if (pSummoned->GetEntry() == NPC_CRYPT_GUARD)
        {
            pSummoned->CastSpell(pSummoned, SPELL_SELF_SPAWN_10, TRIGGERED_OLD_TRIGGERED, NULL, NULL, m_creature->GetObjectGuid());
